...
export default {
  ...
  initialize(container) {
    ...
    withPluginApi('0.11.2', api => {
      ...
      api.registerConnectorClass('composer-fields', 'composer-topic-custom-field-container2', {
        setupComponent(attrs, component) {
          const model = attrs.model;

          // If the first post is being edited we need to pass our value from
          // the topic model to the composer model.
          if (!isDefined(model[fieldName2]) && model.topic && model.topic[fieldName2]) {
            model.set(fieldName2, model.topic[fieldName2]);
          }

          let props = {
            fieldName2: fieldName2,
            fieldValue2: model.get(fieldName2)
          }
          component.setProperties(Object.assign(props, fieldInputTypes(fieldType)));
        },

        actions: {
          onChangeField(fieldValue2) {
            this.set(`model.${fieldName2}`, fieldValue2);
          }
        }
      });
      ...
      api.registerConnectorClass('edit-topic', 'edit-topic-custom-field-container2', {
        setupComponent(attrs, component) {
          const model = attrs.model;
        ...
        actions: {
          onChangeField(fieldValue2) {
            this.set(`buffered.${fieldName2}`, fieldValue2);
          }
        }
      });
      ...
      api.serializeOnCreate(fieldName2);
      api.serializeToDraft(fieldName2);
      api.serializeToTopic(fieldName2, `topic.${fieldName2}`);
      ...
      api.modifyClass('component:topic-list-item', {
        ...
        customFieldName2: fieldName2,
        customFieldValue2: alias(`topic.${fieldName2}`),
